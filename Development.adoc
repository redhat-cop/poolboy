...
--set=image.apiImage.tagOverride=- \
--set=adminUi.apiImage.repository=$(oc get imagestream poolboy-admin-api -o jsonpath='{.status.tags[?(@.tag=="latest")].items[0].dockerImageReference}') \
| oc apply -f -
--------------------------------------------------------------------------------
